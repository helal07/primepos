...
function mergeItems(items: any[]) {
  ...
  for (const it of items) {
    const key = [it.product_id, it.variation_id ?? "", Number(it.unit_price), Number(it.discount ?? 0)].join("|");
...
export function SaleInvoice({ sale, items, settings, onPrint, payments = [], previousDue = 0 }: SaleInvoiceProps) {
  ...
  const terms =
    ...
    "Goods once sold will not be taken back without valid reason. Warranty as per product terms.";
  ...
  const BusinessInfo = ({ align }: { align: "left" | "right" | "center" }) => (
    <div style={{ textAlign: align }}>
      <h1 style={{ fontSize: `${headingSize}px`, fontWeight: "bold", margin: 0, color: headerTextColor }}>
        {businessName}
      </h1>
      {showAddress && businessAddress && (
        <p style={{ fontSize: "12px", color: subTextColor, margin: "2px 0" }}>{businessAddress}</p>
      )}
  ...
  const renderHeaderInner = () => {
    ...
    if (template === "modern") {
      return (
        <div
          style={{ display: "grid", gridTemplateColumns: `1fr ${logoSize + 10}px`, gap: "16px", alignItems: "center" }}
        >
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ fontSize: "11px", color: subTextColor, margin: "2px 0" }}>
              {[
                ...
                showWebsite && businessWebsite,
                showTaxId && businessTax && `TIN/VAT: ${businessTax}`,
              ]
                .filter(Boolean)
                .join(" · ")}
            </p>
    ...
    return (
      <div
        style={{ display: "grid", gridTemplateColumns: `${logoSize + 10}px 1fr`, gap: "16px", alignItems: "center" }}
      >
        <LogoBox />
  ...
  return (
    <div className="p-6" style={{ fontFamily }}>
      ...
      <div id="invoice-print-area">
        {/* Header */}
        <div style={headerWrapStyle}>{renderHeaderInner()}</div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px", marginBottom: "12px" }}>
          <div>
            ...
            <p style={{ fontWeight: "bold", margin: "2px 0", fontSize: "13px" }}>
              {sale.customers?.name || "Walk-in Customer"}
            </p>
            {sale.customers?.address && <p style={{ fontSize: "12px", margin: "2px 0" }}>{sale.customers.address}</p>}
            {sale.customers?.phone && (
              <p style={{ fontSize: "12px", margin: "2px 0" }}>Contact No.: {sale.customers.phone}</p>
            )}
          </div>
          <div style={{ textAlign: "right" }}>
            ...
            <p style={{ margin: "2px 0", fontSize: "12px" }}>
              Time: {new Date(sale.created_at || sale.sale_date).toLocaleTimeString()}
            </p>
          </div>
        </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "16px", fontSize: "12px" }}>
          ...
          <tbody>
            {merged.map((item: any, idx: number) => {
              const lineTotal =
                Number(item.unit_price) * Number(item.quantity) * (1 - Number(item.discount ?? 0) / 100);
              return (
        ...
        >
          <div>
            {payments.length > 0 && (
              <>
                <div
                  style={{ background: ACCENT, color: "#fff", padding: "6px 10px", fontSize: "12px", fontWeight: 600 }}
                >
                  Payments
                </div>
          ...
          <div>
            <div style={{ background: ACCENT, color: "#fff", padding: "6px 10px", fontSize: "12px", fontWeight: 600 }}>
              Amounts
            </div>
            ...
            <div style={{ padding: "0", fontSize: "12px" }}>
              ...
              {Number(sale.discount_amount) > 0 && (
                ...
                >
                  ...
                  <span>
                    - ৳{" "}
                    {Number(sale.discount_amount).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
              ...
              {Number(sale.tax_amount) > 0 && (
                ...
                >
                  ...
                  <span>
                    ৳{" "}
                    {Number(sale.tax_amount).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
              ...
              {Number(sale.shipping_cost) > 0 && (
                ...
                >
                  ...
                  <span>
                    ৳{" "}
                    {Number(sale.shipping_cost).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </span>
              ...
              {prevDue > 0 && (
                <>
                  ...
                  >
                    ...
                    <span>
                      + ৳ {prevDue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>
                  <div
                    style={{
        ...
        <div style={{ marginTop: "12px" }}>
          ...
          <div style={{ padding: "8px 10px", fontSize: "11px", color: "#374151", whiteSpace: "pre-wrap" }}>{terms}</div>
        </div>
