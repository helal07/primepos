...
async function fetchSaleIdsInRange(from: string, to: string, locationId: string): Promise<string[] | null> {
  if (!locationId) return null; // no filter
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const filter: Record<string, any> = {
        created_at: { gte: from, lte: to + "T23:59:59" },
      };
      if (saleIds !== null) {
        if (!saleIds.length) return [];
        filter.sale_id = { in: saleIds.join(",") };
      }
      const items = await rest.all<any>("sale_items", {
        filter, with: ["product"], perPage: 5000,
      });
      const map: Record<string, { name: string; qty: number; revenue: number }> = {};
      items.forEach((item: any) => {
        const pid = item.product_id;
        if (!map[pid]) map[pid] = { name: item.product?.name || "Unknown", qty: 0, revenue: 0 };
