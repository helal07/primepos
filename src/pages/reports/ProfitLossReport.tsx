...
export default function ProfitLossReport() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fetch sales, purchases, products in parallel
      ...
      // COGS = Total Purchase - Closing Stock (purchase price)
      ...
      {isLoading ? <Skeleton className="h-96 w-full" /> : data && (
        <>
            ...
            <Card>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold">{fmt(data.totalSales)}</p>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold">{fmt(data.totalPurchase)}</p>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-lg font-bold ${data.grossProfit >= 0 ? "text-emerald-600" : "text-destructive"}`}>{fmt(data.grossProfit)}</p>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className={`text-lg font-bold ${data.netProfit >= 0 ? "text-emerald-600" : "text-destructive"}`}>{fmt(data.netProfit)}</p>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="text-base">Stock & Returns</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
            ...
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground/70">Total Purchase − Closing Stock (Purchase Price)</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground/70">Total Sales − COGS</p>
                </div>
                <Badge variant={data.grossProfit >= 0 ? "default" : "destructive"} className="text-base px-3 py-1">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground/70">Gross Profit − Total Expenses ({fmt(data.totalExpenses)})</p>
                </div>
                <Badge variant={data.netProfit >= 0 ? "default" : "destructive"} className="text-lg px-4 py-1.5">
                  {fmt(data.netProfit)}
                </Badge>
              </div>
            </CardContent>
...
function Row({ label, value, variant }: { label: string; value: string; variant?: "green" | "red" }) {
  return (
    <div className="flex items-center justify-between text-sm">
      ...
      <span className={`font-medium ${variant === "green" ? "text-emerald-600" : variant === "red" ? "text-destructive" : ""}`}>
        {value}
      </span>
    </div>
  );
