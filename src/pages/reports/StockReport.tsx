...
export default function StockReport() {
  ...
  const exportData = useMemo(() => ({
    columns: ["Product", "SKU", "Category", "Stock", "Purchase Price", "Sell Price", "Stock Value"],
    rows: filtered.map((p: any) => [p.name, p.sku || "-", (p.categories as any)?.name || "-", p.stock_quantity, Number(p.purchase_price).toLocaleString(), Number(p.selling_price).toLocaleString(), (p.stock_quantity * Number(p.purchase_price)).toLocaleString()]),
        ...
        {isLoading ? <Skeleton className="h-64 w-full" /> : (
          <>
            ...
            <Card>
              ...
              <CardContent><div className="overflow-auto max-h-[500px]">
                <Table><TableHeader><TableRow><TableHead>Product</TableHead><TableHead>SKU</TableHead><TableHead>Category</TableHead><TableHead className="text-right">Stock</TableHead><TableHead className="text-right">Purchase Price</TableHead><TableHead className="text-right">Sell Price</TableHead><TableHead className="text-right">Stock Value</TableHead></TableRow></TableHeader>
                <TableBody>{filtered.map((p: any) => (
                  <TableRow key={p.id}><TableCell className="font-medium">{p.name}</TableCell><TableCell className="text-muted-foreground">{p.sku || "—"}</TableCell><TableCell>{(p.categories as any)?.name || "—"}</TableCell>
                    <TableCell className="text-right">{p.stock_quantity <= 0 ? <Badge variant="destructive">0</Badge> : p.stock_quantity <= p.alert_quantity ? <Badge className="bg-amber-500">{p.stock_quantity}</Badge> : <span className="font-medium">{p.stock_quantity}</span>}</TableCell>
                    <TableCell className="text-right">৳{Number(p.purchase_price).toLocaleString()}</TableCell><TableCell className="text-right">৳{Number(p.selling_price).toLocaleString()}</TableCell><TableCell className="text-right font-medium">৳{(p.stock_quantity * Number(p.purchase_price)).toLocaleString()}</TableCell>
                  </TableRow>
                ))}</TableBody></Table>
              </div></CardContent>
            </Card>
